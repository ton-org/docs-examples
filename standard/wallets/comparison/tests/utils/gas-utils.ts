...
export function getFwdStats<T extends Transaction>(transaction: T) {
  if (transaction.description.type !== 'generic') throw new Error('Expected generic transaction');
  if (transaction.description.actionPhase == undefined) throw new Error('Action phase expected');
...
export function computeMessageForwardFees(msgPrices: MsgPrices, msg: Message) {
  ...
  if (msg.info.forwardFee == defaultFwd) {
    return {
      fees: {
        total: msgPrices.lumpPrice,
        res: msgPrices.lumpPrice - defaultFwd,
